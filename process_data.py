...
with open('./data/save_result.txt', 'r') as file:
    content = file.readlines()
i = 0
file_name = "output.csv"
# Mở một file mới để lưu kết quả
with open(file_name, mode='w', newline='') as file:
    writer = csv.writer(file)
    # Lặp qua từng dòng trong nội dung đọc được
    writer.writerow(["PSNR", "std PSNR", "SSIM", "std SSIM", "Exposure", "PE", "std PE"])
    for line in content:
        ...
        if std_psnr_match and std_psnr_match and std_ssim_match and exposure_match and pe_match and std_pe_match:
            ...
            if i==3:
                writer.writerow([psnr_value, std_psnr_value, ssim_value, std_ssim_value, exposure_value, pe_value, std_pe_value])
