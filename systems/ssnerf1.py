...
class SSNeRF1System(BaseSystem):
    ...
    def prepare(self):
        self.criterions = {"psnr": PSNR(), "ssim": ssim}
    ...
    def preprocess_data(self, batch, stage):
        ...
        if stage in ["train"]:
            ...
            fg_mask = (
                self.dataset.all_fg_masks[index.to("cpu")].view(-1).to(self.rank)
            )  # sua cho colmap

        rays = torch.cat(
            [rays_o, F.normalize(rays_d, p=2, dim=-1)], dim=-1
        )  # [8192, 6]
        ...
        batch.update(
            {"rays": rays, "rgb": rgb, "fg_mask": fg_mask, "bright_ness": bright_ness}
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        out = self(
            batch
        )  # ['comp_rgb', 'opacity', 'depth', 'rays_valid', 'num_samples', 'weights', 'points', 'intervals', 'ray_indices']

        bright_ness_predict = out["bright_ness"]
        bright_ness_label = batch["bright_ness"]
        delta_exposure = (
            abs(bright_ness_predict - bright_ness_label) * 100 / bright_ness_label
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ssim = self.criterions["ssim"](
            image_array1, image_array2, multichannel=True, full=True
        )
        ...
        if batch_idx == 0:
            self.save_image_grid(
                ...
                [
                    {
                        "type": "rgb",
                        "img": image_predict.view(H, W, 3),
                        "kwargs": {"data_format": "HWC"},
                    },
                    {
                        "type": "rgb",
                        "img": color_predict.view(H, W, 3),
                        "kwargs": {"data_format": "HWC"},
                    },
                    {
                        "type": "grayscale",
                        "img": density_predict.view(H, W),
                        "kwargs": {},
                    },
                ],
            )
            ...
            for step_out in out:
                ...
                if step_out["index"].ndim == 1:
                    if int(step_out["psnr"]) != 0.0:
                        out_set_psnr[step_out["index"].item()] = {
                            "psnr": step_out["psnr"]
                        }
                        out_set_ssim[step_out["index"].item()] = {
                            "ssim": torch.tensor(step_out["ssim"])
                        }
                        list_delta_exposure.append(step_out["delta_exposure"])
                        list_exposure.append(step_out["exposure_predict"])
                        num_imgs += 1
                        self.save_PSNR[step_out["index"].item()] = out_set_psnr[
                            step_out["index"].item()
                        ]
                        self.save_SSIM[step_out["index"].item()] = out_set_ssim[
                            step_out["index"].item()
                        ]
                        self.save_PE[step_out["index"].item()] = step_out[
                            "delta_exposure"
                        ]
                        self.save_Exposure[step_out["index"].item()] = step_out[
                            "exposure_predict"
                        ]

                    else:
                        if step_out["index"].item() in self.save_PSNR:
                            out_set_psnr[step_out["index"].item()] = self.save_PSNR[
                                step_out["index"].item()
                            ]
                            out_set_ssim[step_out["index"].item()] = self.save_SSIM[
                                step_out["index"].item()
                            ]
                            list_delta_exposure.append(
                                self.save_PE[step_out["index"].item()]
                            )
                            list_exposure.append(
                                self.save_Exposure[step_out["index"].item()]
                            )
                            num_imgs += 1
                # DDP
                else:
                    for oi, index in enumerate(step_out["index"]):
                        if int(step_out["psnr"][oi]) != 0.0:
                            out_set_psnr[index[0].item()] = {
                                "psnr": step_out["psnr"][oi]
                            }
                            out_set_ssim[index[0].item()] = {
                                "ssim": torch.tensor(step_out["ssim"][oi])
                            }
                            check_ssim[f"r_{step_out['index'].item()}.png"] = {
                                torch.tensor(step_out["ssim"][oi])
                            }
                            list_delta_exposure.append(step_out["delta_exposure"])
                            list_exposure.append(step_out["exposure_predict"])
                            num_imgs += 1
